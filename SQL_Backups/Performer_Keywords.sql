...
CREATE TABLE `Performer_Keywords` (
  `performer_keyword_id` int(11) NOT NULL AUTO_INCREMENT,
  `performer_id` bigint(20) unsigned NOT NULL,
  `keyword_id` int(11) NOT NULL,
  PRIMARY KEY (`performer_keyword_id`),
  KEY `performer_id` (`performer_id`),
  KEY `keyword_id` (`keyword_id`),
  CONSTRAINT `Performer_Keywords_ibfk_2` FOREIGN KEY (`keyword_id`) REFERENCES `Keywords` (`keyword_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `Performer_Keywords_ibfk_3` FOREIGN KEY (`performer_id`) REFERENCES `wp_users` (`ID`) ON DELETE CASCADE ON UPDATE CASCADE
